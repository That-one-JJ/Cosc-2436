...
void List::setCapacity(  int newCapacity, const List & origList)
{
    if (newCapacity < mySize)
        newCapacity = mySize;
    if (newCapacity == myCapacity)
        return;
    List newlist(newCapacity, origList );
...
void List::addrandint( int num_ofints ){
    ...
    if(num_ofints < maxcapacity){
        for (int i = mySize + 1; i <= num_ofints; i++){
            int n = 998;
            myArray[i] = rand() % n + 1;
...
void List::insert(ElementType item, int pos, const List & origList )
{
   if (mySize == myCapacity)
   {
       int newCapacity = 2 * myCapacity;
       setCapacity( newCapacity, origList );
   }
